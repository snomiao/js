...
export const 列序映 = (函: (值: 值, 序: number) => any) => (列: 列) =>
  列.map((值, 序) => 函(值, 序));
export const 列归 =
  (函: 归函 = (e) => e, 初值?: 值) =>
  (列: 列) =>
...
export const 表值映 = (函: (值: 值) => any) => (表: 表) =>
  对列表(表对列(表).map(([名称, 位置]) => [名称, 函(位置)]));
export const 表值筛 = (函: (值: 值) => boolean) => (表: 表) =>
  对列表(表对列(表).filter(([, 位置]) => 函(位置)));
export const 表列值列 = (表列: 表列) => 列映(表值列)(表列);
export const 表列填充翻转 = (表列: 表列) =>
  对列表(表列键交(表列).map((键) => [键, 表列.map((表) => 表[键])]));
export const 表列提取 = (表列: 表列, 字段列表: string | string[]) =>
  对列表(表对列(表列).filter(([k]) => 字段列表.includes(k)));
export const 表列翻转 = (表列: 表列) =>
  对列表(表列键交(表列).map((键) => [键, 表列.map((表) => 表[键]).filter((e) => e !== undefined)]));
export const 表列键交 = (表列: 表列) => 列交(表列键列(表列));
export const 表列键列 = (表列: 表列) => 列映(表键列)(表列);
export const 表对列 = (表: 表) => Object.entries(表);
export const 表投影 = (表: 表, 字段列表: string | string[]) =>
  对列表(表对列(表).filter(([k]) => 字段列表.includes(k)));
export const 表键值反转 = (表: 表) =>
  求于(
    表,
    表键值映(([键, 值]) => [值, 键]),
  );
export const 表键值映 = (函: (键值: [string, any]) => [string, any]) => (表: 表) =>
  对列表(表对列(表).map(函));
export const 表键值筛 = (函: (键值: [string, any]) => boolean) => (表: 表) =>
  对列表(表对列(表).filter(函));
...
export const 表键映 = (函: (键: 键) => any) => (表: 表) =>
  对列表(表对列(表).map(([名称, 位置]) => [函(名称), 位置]));
export const 表键筛 = (函: (键: 键) => boolean) => (表: 表) =>
  对列表(表对列(表).filter(([名称]) => 函(名称)));
