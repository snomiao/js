...
const _合集增强表 = 合集 => ({
  ...
  单补: async (补表, 索引 = {
    ...
    return await 合集.updateOne(Object.fromEntries(Object.keys(索引).map(键 => [键, 补表[键]])), {
      $set: 补表
    }, {
      upsert: true,
      ...选项
  ...
  单增改: (查询, 更新, options, cb) => 合集.updateOne(查询, 更新, {
    upsert: true,
    ...options
  }, cb),
  upsertOne: (查询, 更新, options, cb) => 合集.updateOne(查询, 更新, {
    upsert: true,
    ...options
  }, cb),
  多增: 合集.insertMany,
  ...
  多增改: (查询, 更新, options, cb) => 合集.updateMany(查询, 更新, {
    upsert: true,
    ...options
  }, cb),
  upsertMany: (查询, 更新, options, cb) => 合集.updateMany(查询, 更新, {
    upsert: true,
    ...options
  }, cb),
  聚合: 合集.aggregate,
    ...
    for await (const doc of 合集.aggregate(pipeline)) {
      ...
      q.add(async () => {
        try {
          ...
          UpdateFilter && (await 合集.updateOne({
            _id: doc._id
          }, UpdateFilter).catch(e => {
            throw new Error(`错误：在合集 ${合集.collectionName} 尝试更新错误: ${JSON.stringify(UpdateFilter)}, 具体错误内容${e.message}`);
          }));
        } catch (err) {
          if (止于错) throw err;else 错误列.push(err);
        }
      });
      index++;
    }
  ...
  parallelAggregateUpdate: async (pipeline, 更新函数, {
    concurrency = 1,
    stopOnErrors = true,
    showErrors = true
  } = {}) => {
    ...
    for await (const doc of 合集.aggregate(pipeline)) {
      ...
      q.add(async () => {
        try {
          ...
          if (stopOnErrors) throw err;else 错误列.push(err);
        }
      });
      index++;
    }
  ...
  并行各改: async (func, {
    ...
    for await (const doc of 合集.aggregate([$match && {
      $match
    }, $sample && {
      $sample
    }, $sort && {
...
export { snoMongoKu as default, 不早于, 不晚于, 早于, 晚于 };
